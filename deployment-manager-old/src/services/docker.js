...
import { getActiveDeployments, updateDeploymentContainer } from './database';
...
export async function getAppEnvVars(appName, branch = 'main', filterPrefix = null) {
  const query = `
    SELECT key, value 
...
export async function ensureDockerfile(appDir, appName) {
...
export async function buildImage(appName, version) {
  try {
...
export async function startContainer(containerName, imageTag, networkName, envString) {
...
export async function buildAndStartContainer(appName, version, env = {}) {
  try {
    const appDir = path.join(APPS_DIR, appName);
...
export async function waitForHealthyContainer(containerId, timeout = 30000) {
...
export async function recoverContainers() {
  try {
...
export async function deleteAppContainers(appName) {
  try {
